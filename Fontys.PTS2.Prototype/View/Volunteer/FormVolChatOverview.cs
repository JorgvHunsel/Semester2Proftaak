    ...
    public partial class FormVolChatOverview : Form
    {
        ...
        private void btnOpenChat_Click(object sender, EventArgs e)
        {
            if (lvChats.SelectedItems.Count == 1)
            {
                ...
                ((FormMain)this.Parent.Parent).ReplaceForm(new FormVolChat(selectedChatId, selectedName, FormLogin.currentUser.UserId, receiverID));
            }
            else
            {
        ...
        private void btnMakeAppointment_Click(object sender, EventArgs e)
        {
            if (lvChats.SelectedItems.Count == 1)
            {
                ...
                ((FormMain)this.Parent.Parent).ReplaceForm(new FormVolAppointment(questionID, receiverID, FormLogin.currentUser.UserId, selectedName));
            }
            else
            {
                MessageBox.Show("Error");
